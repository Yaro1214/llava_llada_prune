...
class Random(LLaDAModel):
    def __init__(self, config: LLaDAConfig):
        self.last_attention = None
        self.retained_indices = None
        # self.flag2pruning = 0 # 1 means have pruned, 0 means not pruned
        super().__init__(config)
    ...
    def forward(
        ...
        gen_length = self.config.gen_length if self.config.gen_length is not None else 0
        suffix_len = self.config.suffix_len if self.config.suffix_len is not None else 0
        if Pruning_config is not None:
            num_block = Pruning_config['num_block']
            num_step = Pruning_config['num_step']
            K = Pruning_config['K']  # pruned layer
            ratio = Pruning_config['reduction_ratio']
            image_start = Pruning_config['image_token_start_index']
            image_token_length = Pruning_config['image_token_length'] if self.config.text_length is None else (seq_length - self.config.text_length - gen_length - suffix_len)
...
class RandomV2(LLaDAModel):
    def __init__(self, config: LLaDAConfig):
        self.last_attention = None
        self.retained_indices = None
        self.pruned_indices = None
        self.pruned_tokens = None
        self.n = 2
        # self.flag2pruning = 0 # 1 means have pruned, 0 means not pruned
        super().__init__(config)
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            print("Layer: ", decoder_layer.self_attn.layer_idx)
            print(hidden_states.shape)
            if output_hidden_states:
                all_hidden_states += (hidden_states,)


            if decoder_layer.self_attn.layer_idx >= K and (decoder_layer.self_attn.layer_idx - K) % self.n == 1 and decoder_layer.self_attn.layer_idx < 11 and seq_length > 1 and num_block == 0 and num_step == 0:
                #print("Restoring pruned tokens at layer: ", decoder_layer.self_attn.layer_idx)
                device = hidden_states.device
                dtype = hidden_states.dtype
                # 1. 构建完整索引列表
                # all_indices = torch.cat((
                #     torch.arange(0, image_start, device=device),
                #     torch.tensor(self.pruned_indices, device=device),  # 恢复被剪枝的索引
                #     torch.tensor(self.retained_indices.squeeze(), device=device)[image_start:],  # 保留的索引
                #     torch.arange(image_end, seq_length, device=device)
                # )).sort().values.long()
                
                # 2. 恢复完整的hidden_states
                batch_size, hidden_dim = hidden_states.shape[0], hidden_states.shape[2]
                restored_hidden = torch.zeros(batch_size, seq_length, hidden_dim, device=device, dtype=dtype)
                
                # 填充保留的token（剪枝后保留的部分）
                restored_hidden[:, self.retained_indices.squeeze(), :] = hidden_states
                # 恢复被剪枝的token（新增）
                restored_hidden[:, self.pruned_indices, :] = self.pruned_tokens
                
                hidden_states = restored_hidden
                
                # 3. 更新位置索引和掩码
                cache_position = torch.arange(seq_length, device=device)
                position_ids = cache_position.unsqueeze(0)
                causal_mask = self._update_causal_mask(None, hidden_states, cache_position, is_causal=False)
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if decoder_layer.self_attn.layer_idx >= K and (decoder_layer.self_attn.layer_idx - K) % self.n == 0 and decoder_layer.self_attn.layer_idx < 10 and seq_length > 1 and num_block == 0 and num_step == 0:
                    #print("Pruning layer: ", decoder_layer.self_attn.layer_idx, " with ratio: ", ratio)
                    device = hidden_states.device
                    all_image_indices = list(range(image_start, image_end))

                    image_indexs = random.sample(range(image_start, image_end), int((1 - ratio) * image_token_length))

                    pruned_indexs = [idx for idx in all_image_indices if idx not in image_indexs]
                    self.pruned_tokens = hidden_states[:, pruned_indexs, :].clone()
                    self.pruned_indices = pruned_indexs

                    keep_indexs = torch.cat((torch.arange(0, image_start, device=device), torch.Tensor(image_indexs).to(device), torch.arange(image_end, seq_length, device=device)))
                    keep_indexs = keep_indexs.sort().values.long()
                    #print("Keep indexs: ", keep_indexs)
                    #print("Pruned indexs: ", self.pruned_indices)
                    # save keep_indexs
...
class RandomV3(LLaDAModel):
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            # print("Layer: ", decoder_layer.self_attn.layer_idx)
            # print(hidden_states.shape)
            # print(num_block, num_step)
            if output_hidden_states:
                all_hidden_states += (hidden_states,)


            if decoder_layer.self_attn.layer_idx in [4,7,10,13,16] and seq_length > 1 and num_block == 0:
                #print("Restoring pruned tokens at layer: ", decoder_layer.self_attn.layer_idx)
                device = hidden_states.device
                dtype = hidden_states.dtype
                # 1. 构建完整索引列表
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if decoder_layer.self_attn.layer_idx in [2,5,8,11,14] and seq_length > 1 and num_block == 0:
                    #print("Pruning layer: ", decoder_layer.self_attn.layer_idx, " with ratio: ", ratio)
                    device = hidden_states.device
                    all_image_indices = list(range(image_start, image_end))
...
class DART(LLaDAModel):
    ...
    def forward(
        ...
        if Pruning_config is not None:
            ...
            image_start = Pruning_config['image_token_start_index']
            image_token_length = Pruning_config['image_token_length'] if self.config.text_length is None else (seq_length - self.config.text_length - gen_length - suffix_len)
            Pruning_config['image_token_length'] = image_token_length
            image_end = image_start + image_token_length
            # pdb.set_trace()
        else:
            K = -1
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if decoder_layer.self_attn.layer_idx == K and seq_length > 1 and num_block == 0 and num_step == 0:
                    ...
                    keep_indexs = torch.cat((torch.arange(0, image_start, device=device), retained_image_tokens_index, torch.arange(image_end, seq_length, device=device)))
                    # sort index
    ...
    def get_retained_image_token(self, config: LLaDAConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        Pruning_config = config.Pruning_config
        K = Pruning_config['K']  # pruned layer
        image_token_start_index = Pruning_config['image_token_start_index']
        image_token_length =Pruning_config['image_token_length']

        pivot_image_token = Pruning_config['pivot_image_token']
        pivot_text_token = Pruning_config['pivot_text_token']


        reduction_ratio = Pruning_config['reduction_ratio']
        TOKEN_TOPK = math.ceil((image_token_length * (1 - reduction_ratio)) // (pivot_image_token + pivot_text_token))

        device = last_layer_state.device
        # pdb.set_trace()
        # any_states = any_states.permute(0, 2, 1, 3).reshape(any_states.shape[0], any_states.shape[2], -1)
        
        any_states_image_token = any_states[0][image_token_start_index:image_token_start_index + image_token_length, :]
        any_states_query_token = any_states[0][image_token_start_index + image_token_length: last_layer_state.size(1) - config.gen_length - config.suffix_len, :]
        # pdb.set_trace()

        any_states_image_token_L1_norm = torch.norm(any_states_image_token, p=1, dim=-1)
        any_states_query_token_L1_norm = torch.norm(any_states_query_token, p=1, dim=-1)

        image_indices = (any_states_image_token_L1_norm.topk(pivot_image_token).indices + image_token_start_index).tolist() 
        query_indices = (any_states_query_token_L1_norm.topk(pivot_text_token).indices + image_token_start_index + image_token_length).tolist()
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if decoder_layer.self_attn.layer_idx == K and seq_length > 1 and num_block == 0 and num_step == 0:
                    ...
                    image_attention_score = self.last_attention.mean(dim=1)[0][seq_length - gen_length - suffix_len][image_start:image_end]   # TODO: for diffusion-based mllm
                ...
                if decoder_layer.self_attn.layer_idx == K - 1 and seq_length > 1:
...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def generate_with_embeds(self, inputs_embeds, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            for num_block in range(num_blocks):
                if self.model.config.Pruning_config is not None:
                    self.model.config.Pruning_config['num_block'] = num_block
                ...
                for i in range(steps):
                    if self.model.config.Pruning_config is not None:
                        self.model.config.Pruning_config['num_step'] = i
